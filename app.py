...
Endpoints:
  GET  /                 -> dashboard UI
  POST /api/simulate     -> generate N traffic flows, run them through the
                             ML detector + smart contract, append non-benign
                             events to the blockchain, return the results
  GET  /api/state        -> current dashboard state (recent flows, ledger,
                             blacklist, chain validity)
...
recent_flows = []   # rolling log of recent flow classifications (for the UI)
MAX_RECENT = 60
...
@app.route("/api/state")
def state():
    blacklisted = {sid: n for sid, n in contract._strikes.items() if n >= 2}
    return jsonify({
...
def reset():
    global chain, contract, recent_flows
    chain = Blockchain()
    contract = MitigationContract()
    recent_flows = []
